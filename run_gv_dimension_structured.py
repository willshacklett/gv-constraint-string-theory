...
def simulate(dim, size, C, inj, steps=600, lam=0.08, seed=42):
    ...
    if dim == 1:
        gv = 0.95 + 0.02 * rng.standard_normal(size)
        lap_fn = laplacian_1d
    elif dim == 2:
        gv = 0.95 + 0.02 * rng.standard_normal((size, size))
        lap_fn = laplacian_2d
    else:
        gv = 0.95 + 0.02 * rng.standard_normal((size, size, size))
        lap_fn = laplacian_3d

    gv = np.clip(gv, 0, 1)

    # heterogeneous constraint field
    constraint = C + 0.03 * rng.standard_normal(gv.shape)
    ...
    # structured survival rule (not mean-field)
    survives = (collapse_fraction < 0.5) and (mean_gv > 0.4)
...
def main():
    ...
    print("Running STRUCTURED dimension sweep...\n")
