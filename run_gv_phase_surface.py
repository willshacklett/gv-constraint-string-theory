...
def simulate(C, S, k, inj, size=48, steps=250, lam=0.08, seed=42):
...
def find_inj_min(C, S, k):
    inj_vals = np.linspace(0.01, 2.0, 60)  # higher resolution than before
...
def main():
    # Reduced grid (fast but still meaningful)
    C_vals = np.linspace(0.2, 2.0, 6)
    S_vals = np.linspace(0.0, 2.0, 5)
    k_vals = [2, 4, 6, 8, 10]

    rows = []

    print("Running FAST phase surface sweep...\n")
